...
from submission.nodes.validate_input import validate_input_node
from submission.nodes.draft_proposal import draft_proposal_node
from submission.nodes.prepare_approval import prepare_approval_node
from submission.nodes.revalidate import revalidate_node
from submission.nodes.execute import execute_node
from submission.nodes.audit import audit_node
from submission.agents.investigation import investigation_agent_node
from submission.agents.sourcing import sourcing_agent_node
from submission.agents.review import review_agent_node
from submission.config import MAX_REVISION_COUNT
...
def route_after_validation(state: InventraState) -> str:
...
def route_after_review(state: InventraState) -> str:
...
def route_after_execution(state: InventraState) -> str:
...
def revision_bump_node(state: InventraState) -> dict:
    """Increment the revision counter before looping back to sourcing."""
    return {"revision_count": state.get("revision_count", 0) + 1}


def human_approval_node(state: InventraState) -> dict:
...
def build_graph(checkpointer=None):
    """Build and compile the Vitrious LangGraph workflow.
    ...
    graph = StateGraph(InventraState)
