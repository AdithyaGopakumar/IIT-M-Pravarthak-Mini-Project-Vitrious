...
| **Input** | `sku`, `warehouse_id`, `target_cover_days` (from `InventraState`) |
...
See [`state.py`](file:///d:/Learnings/IIT-M%20Agentic%20AI/Projects/Proquirement%20-%20Mini%20project/Vitrious/submission/state/state.py). The state uses `TypedDict` and is serializable (all Pydantic models are dumped to dicts) so LangGraph can persist it across the human interrupt.
...
All thresholds are stored in [`config.py`](file:///d:/Learnings/IIT-M%20Agentic%20AI/Projects/Proquirement%20-%20Mini%20project/Vitrious/submission/config.py) and loaded from environment variables. None are hidden inside prompts.
